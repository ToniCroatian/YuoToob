...
if($_SERVER['REQUEST_URI'] == "/rss/global/recently_added.rss" || $_SERVER['REQUEST_URI'] == "/rss/") {
	...
	$feed_link = $siteurl."/rss/global/recently_added.rss";
	...
	if($user->rowCount() > 0) {
		...
		$feed_description = "Videos uploaded by ".htmlspecialchars($user['username'])." hosted at ".$siteurl.".";
		$feed_link =  $siteurl."/rss/user/".htmlspecialchars($user['username'])."/my_videos.rss";
	...
	$feed_link = $siteurl."/rss/tag/".urlencode($matches['0']['1']).".rss";
} else {
...
<rss version="2.0" xmlns:media="http://search.yahoo.com/mrss">
	<channel>
		<title>YuoToob :: <?php echo $feed_title; ?></title>
		<link><?php echo $feed_link; ?></link>
		<description><?php echo $feed_description; ?></description>
		<?php foreach ($videos as $video) { ?>
		<item>
			<author>rss@<?php echo $_SERVER["SERVER_NAME"]; ?> (<?php echo htmlspecialchars($video['username']); ?>)</author>
			<title><?php echo htmlspecialchars($video['title']); ?></title>
			<link><?php echo $siteurl; ?>/?v=<?php echo htmlspecialchars($video['vid']); ?></link>
			<description><![CDATA[
				<img src="<?php echo $siteurl; ?>/get_still.php?video_id=<?php echo htmlspecialchars($video['vid']); ?>" align="right" border="0" width="120" height="90" vspace="4" hspace="4" />
				<p><?php echo htmlspecialchars($video['description']); ?></p>
				<p>
					Author: <a href="<?php echo $siteurl; ?>/profile.php?user=<?php echo htmlspecialchars($video['username']); ?>"><?php echo htmlspecialchars($video['username']); ?></a><br/>
					Keywords: <?php foreach(explode(" ", $video['tags']) as $tag) echo '<a href="'.$siteurl.'/results.php?search='.htmlspecialchars($tag).'">'.htmlspecialchars($tag).'</a> '; ?><br/>
					Added: <?php echo retroDate($video['uploaded']); ?><br/>
				</p>
			]]></description>
			<guid isPermaLink="true"><?php echo $siteurl; ?>/?v=<?php echo htmlspecialchars($video['vid']); ?></guid>
			<pubDate><?php echo retroDate($video['uploaded'], "r"); ?></pubDate>

			<media:player url="<?php echo $siteurl; ?>/?v=<?php echo htmlspecialchars($video['vid']); ?>" />
			<media:thumbnail url="<?php echo $siteurl; ?>/get_still.php?video_id=<?php echo htmlspecialchars($video['vid']); ?>" width="120" height="90" />
